			...
			for (int i = 1; i < size().first; ++i) {
				...
				while(cost > error && iters < i){
					...
					for(int k=i-1; k>=0; --k){
						...
						if (dimension == 0 && rowDeps.count(k) != 0) continue;
						if (dimension == 1 && colDeps.count(k) != 0) continue;
					...
					for(int k=0; k<basisMapping.grid.size(); ++k){
						if(abs(basisMapping.grid[k][0]) > epsilon) weights.push_back({rowsInBasis[k],basisMapping.grid[k][0]});
					}
				...
				if (cost < error && weights.size() < b.size().first) {
					if (dimension == 0) rowDeps[i] = weights;
        ...
        for(int j=0; j<cols; ++j){
            ...
            if(left.rowDeps.count(i) || right.colDeps.count(j)){
                ...
                for(heavy_matrix::dep& comb : deps){
                    ...
                    if(left.rowDeps.count(i)) dest.at(i,j) += comb.scalar * dest.at(comb.index,j);
                    else dest.at(i,j) += comb.scalar * dest.at(i,comb.index);
                }
                floating_adds--;
            }
